...
export default function Header({ onMenuClick, totalEarnings }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "balanceUpdated",
      // 使用 as EventListener 是 TypeScript 的类型断言，确保 handleBalanceUpdate 符合事件监听器的类型。
      handleBalanceUpdate as EventListener
    );

    /**
     * return () => { ... }：这部分通常是在 React 组件中使用的清理函数。当组件卸载时，这个函数会被调用。
     */
    return () => {
  ...
  const login = async () => {
    ...
    try {
      // 当调用 connect() 方法时，它会处理用户的身份验证流程，并返回一个提供者对象（web3authProvider），该对象可以用于与区块链交互。
